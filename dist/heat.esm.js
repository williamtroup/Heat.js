...
(() => {
    ...
    function h(e, t, n, i) {
        let o = null;
        const s = DomElement.createWithHTML(t, "div", "year-menu-item", n.toString());
        if (e._currentView.year !== n) {
            ...
            if (n === i) {
                DomElement.addClass(s, "year-menu-item-current");
            }
        } else {
            DomElement.addClass(s, "year-menu-item-active");
            o = s;
        }
        return o;
    }
        ...
        if (t.views.map.showNoDataMessageWhenDataIsNotAvailable && !D(t)) {
            ...
            for (let n = 0; n < 12; n++) {
                if (W(t.views.map.monthsToShow, n)) {
                    const i = DomElement.create(r, "div", "month");
                    ...
                    if (t.views.map.showMonthNames) {
                        ...
                        if (Is.defined(o)) {
                            if (t.views.map.showMonthDayGaps) {
                                ...
                                o.style.width = `${s - t._currentView.dayWidth}px`;
                            }
                        }
                    }
    ...
    function b(e) {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            for (let s = 0; s < o; s++) {
                const o = new Date(e._currentView.year, i, s + 1);
                const r = DateTime.toStorageDate(o);
    ...
    function C(e, t, n, i, o) {
        const s = DomElement.create(e, "div", "day-line");
        const r = n * o;
        s.style.height = `${r}px`;
        s.setAttribute("data-heat-js-day-number", t.toString());
        if (r <= 0) {
        ...
        if (Is.definedFunction(i.events.onWeekDayClick)) {
            s.onclick = () => Trigger.customEvent(i.events.onWeekDayClick, t, n, i._currentView.year);
        } else if (Is.definedFunction(i.events.onWeekDayDblClick)) {
            s.onclick = () => Trigger.customEvent(i.events.onWeekDayDblClick, t, n, i._currentView.year);
        } else {
    ...
    function _(e) {
        ...
        for (let i = 0; i < 12; i++) {
            const o = DateTime.getTotalDaysInMonth(e._currentView.year, i);
            for (let s = 0; s < o; s++) {
                const o = DateTime.toStorageDate(new Date(e._currentView.year, i, s + 1));
    ...
    function B(e, t) {
        ...
        i.types["0"] = 0;
        for (let o = 0; o < 12; o++) {
            const s = DateTime.getTotalDaysInMonth(e._currentView.year, o);
            for (let r = 0; r < s; r++) {
                const s = DateTime.toStorageDate(new Date(e._currentView.year, o, r + 1));
                if (n.hasOwnProperty(s)) {
                    const o = DateTime.getStorageDate(s);
    ...
    function de(t) {
        ...
        for (const e in n) {
            if (n.hasOwnProperty(e)) {
                i.push(xe([ Te(e), Te(n[e].toString()) ]));
            }
        }
        if (i.length > 0) {
            i.unshift(xe([ Te(e.text.dateText), Te(e.text.countText) ]));
        }
        return i.join("\n");
    }
    function fe(e) {
