...
(function(){function ya(){for(var a=h.domElementTypes,b=a.length,c=0;c<b;c++){var d=F.getElementsByTagName(a[c]);d=[].slice.call(d);for(var e=d.length,f=0;f<e;f++){var g=d[f],l=!0;if(v(g)&&g.hasAttribute("data-heat-options")){var m=g.getAttribute("data-heat-options");q(m)?(m=na(m),m.parsed&&G(m.result)?za(Aa(m.result,g)):h.safeMode||(console.error(h.attributeNotValidErrorText.replace("{{attribute_name}}","data-heat-options")),l=!1)):h.safeMode||(console.error(h.attributeNotSetErrorText.replace("{{attribute_name}}",
"data-heat-options")),l=!1)}if(!l)break}}}function Aa(a,b){var c=Ba(a),d=q(c.view)?c.view.toLowerCase():r.empty;c.currentView={};c.currentView.element=b;c.currentView.tooltip=null;c.currentView.tooltipTimer=null;c.currentView.mapContents=null;c.currentView.mapContentsScrollLeft=0;c.currentView.year=c.year;c.currentView.type=h.unknownTrendText;c.currentView.isInFetchMode=N(c.onDataFetch);c.currentView.isInFetchModeTimer=null;c.currentView.yearsAvailable=[];c.views.chart.enabled&&(c.currentView.chartContents=
null,c.currentView.chartContentsScrollLeft=0);c.views.statistics.enabled&&(c.currentView.statisticsContents=null,c.currentView.statisticsContentsScrollLeft=0);c.currentView.view="map"===d?1:"chart"===d?2:"statistics"===d?3:1;return c}function za(a){w(a.onBeforeRender,a.currentView.element);q(a.currentView.element.id)||(a.currentView.element.id=V());a.currentView.element.className.trim()===r.empty?a.currentView.element.className="heat-js":x(a.currentView.element,"heat-js");a.currentView.element.removeAttribute("data-heat-options");
oa(a.currentView.element.id,a);z(a);w(a.onRenderComplete,a.currentView.element)}function z(a,b,c){b=n(b,!1);c=n(c,!1);if(b&&a.useLocalStorageForData&&B.localStorage){b=a.currentView.element.id;if(a.useLocalStorageForData&&B.localStorage){for(var d=B.localStorage.length,e=[],f=a.currentView.element.id,g=0;g<d;g++)Ca(B.localStorage.key(g),"HJS_"+f)&&e.push(B.localStorage.key(g));d=e.length;for(f=0;f<d;f++)B.localStorage.removeItem(e[f])}e=ba.stringify(k[b].type);B.localStorage.setItem("HJS_"+b,e)}v(a.currentView.mapContents)&&
